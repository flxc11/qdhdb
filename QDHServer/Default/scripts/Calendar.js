    ...
    function drawCld(SY,SM) {
        ...
        for(i=0;i<42;i++) {
            ...
            if(sD>-1 && sD<cld.length) { //日期内
                sObj.innerHTML = sD+1 + "<input type='hidden' class='Crt_day' value='" + (sD+1) + "' />";
                ...
                cld1 = new calendar(SY,SM - 1);
                var sD1 = cld1.length + sD;
                sObj.innerHTML = sD1+1;
                if(cld1[sD1].lDay==1){ //显示农历月
                    lObj.innerHTML = '<b>'+(cld1[sD1].isLeap?'闰':'') + cld1[sD1].lMonth + '月' + (monthDays(cld1[sD1].lYear,cld1[sD1].lMonth)==29?'小':'大')+'</b>';
                }
                else{lObj.innerHTML = cDay(cld1[sD1].lDay);}	//显示农历日
                var Slfw=Ssfw=null;
                s=cld1[sD1].solarFestival;
                for (var ipp=0;ipp<lFtv.length;ipp++){	//农历节日
                    if (parseInt(lFtv[ipp].substr(0,2))==(cld1[sD1].lMonth)){
                        if (parseInt(lFtv[ipp].substr(2,4))==(cld1[sD1].lDay)){
                            lObj.innerHTML=lFtv[ipp].substr(5);
                            Slfw=lFtv[ipp].substr(5);
                        }
                    }
                    if (12==(cld1[sD1].lMonth)){	//判断是否为除夕
                        if (eve==(cld1[sD1].lDay)){lObj.innerHTML="除夕";Slfw="除夕";}
                    }
                }
                for (var ipp=0;ipp<sFtv.length;ipp++){	//公历节日
                    if (parseInt(sFtv[ipp].substr(0,2))==(SM+1)){
...
function initial() {
    ...
    document.getElementById("CurrentTR").innerHTML ='<p class="p_month">'
        + '<span class="cn">' + (Today.getMonth() + 1) + "月" + '</span>'
        + '<span class="en">' + monName[Today.getMonth()] + '</span>'
        + '</p>' +
        '<p class="p_year">' + Today.getFullYear() + '</p>';
    drawCld(tY,tM);
}
